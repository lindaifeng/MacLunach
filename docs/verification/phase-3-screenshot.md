...
尚未完成：滚动截图拼接、GIF 帧录制与编码、翻译；OCR 结果面板尚待真实鼠标/键盘、VoiceOver 和用户实际截图视觉验收；模糊、放大镜、裁剪尚待完整编辑器 UI；钉图尚待透明度、折叠/展开、穿透恢复、重启持久化；另需 6K 连续导出内存门槛以及完整实机/多屏/可访问性验收。滚动截图与 GIF 已进入真实、可取消的协调器扩展路由，但默认路由仍明确返回未完成状态，不得伪装成功。
