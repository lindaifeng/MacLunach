...
- TouchKit 关键全量回归：当前 121 个测试通过；部署目标检查为 macOS 14.0 一致。
...
- 两类问题均已达到本轮排查上限，不再无限重复全量构建。真实鼠标拖动、工具栏视觉、不同缩放多屏、VoiceOver 仍需独立实机记录。
